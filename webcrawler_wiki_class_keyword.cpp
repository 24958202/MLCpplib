		...
		*/
		...
		if(!getContent.empty()){
			...
			if(!content_tokenized_by_sentences.empty()){
				...
				std::vector<std::string> web_key_words = get_key_words(htmlContent);
				if(!web_key_words.empty()){
					for(const auto& wkw : web_key_words){
						std::cout << "The website's Keyword >>: " << wkw << '\n';
						std::string str_folder_name = wkw;

...
//    return size * nmemb;  
//}  
size_t WebSpiderLib::WriteCallback(void* contents, size_t size, size_t nmemb, void* userp) {  
...
std::string WebSpiderLib::GetURLContent(const std::string& url) {  
    ...
    try {  
        ...
        if (curl) {  
            curl_easy_cleanup(curl); // Ensure cleanup happens  
        }  
...
std::vector<std::string> get_all_urls(const std::string& start_url, const std::string& domain) {  
    ...
    while (!to_visit.empty()) {  
        ...
        if (!html_content.empty()) {  
            ...
            std::cerr << "Failed to get content from: " << current_url << std::endl;
        }
    }  
    return all_urls;  
}

...
libann_relu::libann_relu(const std::vector<unsigned int>& layerSizes) {  
    if (layerSizes.empty()) {  
        return;  
    }  
        ...
        for (unsigned int j = 0; j < numNeurons; j++) {  
            ...
            if (i > 0) {  
                ...
                for (unsigned int k = 0; k < numInputs; k++) {  
                    neuron.weights.push_back(randomWeight());  
                }  
            }  
...
std::vector<double> libann_relu::feedForward(const std::vector<double>& inputs) {  
    ...
    for (unsigned int i = 1; i < layers.size(); i++) {  
        for (unsigned int j = 0; j < layers[i].neurons.size(); j++) {  
            ...
            layers[i].neurons[j].value = relu(weightedSum); // Use ReLU activation  
        }  
    }  
...
void libann_relu::train(const std::vector<std::vector<double>>& trainingData, const std::vector<unsigned int>& labels, unsigned int numEpochs, double learningRate, const std::string& file_path, std::function<void(const std::string&)> call_back) {  
...
void libann_relu::loadModel(const std::string& filename) {  

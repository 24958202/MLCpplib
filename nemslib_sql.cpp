...
/*
    Mysqlserver version
*/
int nlp_lib::check_all_voc_nums(const mydb_connection& conn){
    MySQLDatabase db;   
    ...
    std::string count_query = "SELECT COUNT(*) AS rscount FROM nlp_db.all_voc";
...
void nlp_lib::write_books_mysql(const std::string& input_folder_path,const std::string& stopwordListPath,const std::string& output_log_path,const mydb_connection& conn){
                    ...
                    for(const auto& bt : book_token){
                        ...
                        try{
                            ...
                            */
                            ...
                            select_sql << "SELECT * FROM nlp_db.all_voc where t_x = '" << bt << "'";
                            std::string str_s = select_sql.str();
                            const char* query = str_s.c_str();
                            if (db.executeQuery(query)) {
                                MYSQL_RES* result = db.fetchResult();
                                if (result) {
                                    MYSQL_ROW row = mysql_fetch_row(result);
                                    if (row) {
                                        // Access the columns directly since there is only one row
                                        ...
                                        */
                                        ...
                                        //std::string escaped_bt = mysql_real_escape_string(bt); // assuming mysql_real_escape_string is available
                                        insert_sql << "INSERT into nlp_db.all_voc(t_id,t_x)values(" << std::to_string(t_id) << ",'" << bt << "')";
                                        std::string str_insert = insert_sql.str();
                                        db.executeQuery(str_insert.c_str());
                                        book_token_xy.push_back(std::to_string(t_id));
                                        /*
                                            -----------------------------------
                                        */
                                    }
                                    ...
                                    std::stringstream insert_sql_else;
                                    //std::string escaped_bt = mysql_real_escape_string(bt); // assuming mysql_real_escape_string is available
                                    insert_sql_else << "INSERT into nlp_db.all_voc(t_id,t_x)values(" << std::to_string(t_id) << ",'" << bt + "')";
                                    std::string str_insert = insert_sql_else.str();
                                    db.executeQuery(str_insert.c_str());
                                    book_token_xy.push_back(std::to_string(t_id));
                                }
                            }
    ...
    */
    for (const auto& entry : std::filesystem::directory_iterator(input_folder_path)) {
        if (entry.is_regular_file() && entry.path().extension() == ".txt" && this->ProcessFileNames.find(entry.path()) == this->ProcessFileNames.end()){
            this->write_books_mysql(input_folder_path,stopwordListPath,output_log_path,conn);
            return;
        }
    }
    db.closeConnection();
...
std::vector<std::string> nlp_lib::read_books_mysql(const mydb_connection& conn){
    std::string str_book;
    ...
    if (db.executeQuery(query)) {
        ...
        if (result) {
            ...
            std::cerr << "Failed to retrieve result set: " << mysql_error(db.getConnection()) << '\n';
        }
    } else {
